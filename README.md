# Hi, I'm Jai Kapoor 👨‍💻

## Software Engineer | Competitive Programmer | Full-Stack Developer

[![GitHub followers](https://img.shields.io/github/followers/jaipkapoor99?label=Follow&style=social)](https://github.com/jaipkapoor99)
[![LinkedIn](https://img.shields.io/badge/-LinkedIn-blue?style=flat-square&logo=Linkedin&logoColor=white&link=https://www.linkedin.com/in/jaipkapoor/)](https://www.linkedin.com/in/jaipkapoor/)

---

## 🚀 About Me

I'm a passionate software engineer who thrives at the intersection of **algorithmic problem-solving** and **practical application development**. My journey in technology is driven by an insatiable curiosity to understand complex systems and a desire to build solutions that make a meaningful impact.

My competitive programming background has instilled in me a deep appreciation for efficient algorithms and optimal solutions, skills that I seamlessly translate into real-world software development. I believe that the analytical mindset developed through solving 757+ algorithmic challenges has made me a more thoughtful and systematic developer.

- 🔭 Currently building production-ready applications with **React** and **TypeScript**
- 🌱 Exploring **Artificial Intelligence** and **Large Language Models**
- 💪 Solved **757+ algorithmic challenges** across competitive programming platforms
- 🎯 Peak Codeforces rating: **1374 (Pupil)**
- 📍 Based in **Mumbai, India**
- 🎂 Born **July 28, 1999**

---

## 🎓 Education

**🏛️ B.Tech Computer Science** _(2018 - April 2022)_

- 🏫 N.M.I.M.S. University, Mumbai
- 📊 CGPA: 3.11/4.00
- 📚 Relevant Coursework: Data Structures, Algorithms, Database Systems, Software Engineering

**🎓 Higher Secondary Certificate (HSC)** _(12th Grade)_

- 🏫 Nirmala Memorial Foundation College
- 📈 Percentage: 70.77%

---

## 💼 Professional Experience

### 💻 Software Engineer - Flexera (Remote)

_📅 December 2022 - November 2023_

- 🔧 Developed and maintained software solutions in a remote-first environment
- 🤝 Collaborated with cross-functional teams on enterprise-level applications

### 💻 Software Engineer - Lentra (Pune)

_📅 May 2022 - August 2022_

- 💰 Contributed to fintech applications and backend systems
- 🚀 Gained experience in production software development

---

## 🛠️ Featured Projects

### 📱 [Booking Platform](https://github.com/jaipkapoor99/booking-platform) - _Production Ready_

**Technologies:** React, TypeScript, Vite, Supabase, Shadcn

- 🏗️ Full-stack booking and scheduling system
- 🗃️ Comprehensive database schema following 3NF compliance
- 🎨 Responsive front-end with modern UI components
- 📊 User authentication, booking management, and admin dashboard
- **🎯 Outcome:** Led to commercial opportunity with mentor Brian Fialho

### 🎮 [Interactive Tic-Tac-Toe Game](https://github.com/jaipkapoor99/Tic-Tac-Toe)

**Technologies:** JavaScript, HTML5, CSS3

- 🎲 Interactive gameplay with AI opponent
- 🎨 Clean, responsive user interface
- 🧠 Game logic implementation with win condition detection

### 💻 [Competitive Programming Template System](https://github.com/jaipkapoor99/CP-Template)

**Technologies:** C++, Template Metaprogramming

- ⚡ Fast I/O operations and common algorithms
- 🔧 Modular code structure for rapid problem solving
- 🏆 Battle-tested across 56+ Codeforces contests

---

## 🏆 Competitive Programming Stats

### 🟡 Codeforces

[![Codeforces](https://img.shields.io/badge/Codeforces-1374-yellow?style=flat-square&logo=codeforces)](https://codeforces.com/profile/jaipkapoor)

**Peak Rating:** 1374 (Pupil) | **Problems Solved:** 757+ | **Contests:** 56

### 🟢 LeetCode

[![LeetCode](https://img.shields.io/badge/LeetCode-1628-green?style=flat-square&logo=leetcode)](https://leetcode.com/jaipkapoor99/)

**Contest Rating:** 1628 | **Problems Solved:** 130/3580 | **Global Rank:** Top 19.62%

---

## 🛠️ Tech Stack

### 💻 Languages

![C++](https://img.shields.io/badge/-C++-00599C?style=flat-square&logo=c%2B%2B&logoColor=white)
![TypeScript](https://img.shields.io/badge/-TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white)
![JavaScript](https://img.shields.io/badge/-JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![Python](https://img.shields.io/badge/-Python-3776AB?style=flat-square&logo=python&logoColor=white)

### 🎨 Frontend

![React](https://img.shields.io/badge/-React-61DAFB?style=flat-square&logo=react&logoColor=black)
![Next.js](https://img.shields.io/badge/-Next.js-000000?style=flat-square&logo=next.js&logoColor=white)
![Tailwind CSS](https://img.shields.io/badge/-Tailwind%20CSS-38B2AC?style=flat-square&logo=tailwind-css&logoColor=white)
![HTML5](https://img.shields.io/badge/-HTML5-E34F26?style=flat-square&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/-CSS3-1572B6?style=flat-square&logo=css3&logoColor=white)

### ⚙️ Backend & Database

![Node.js](https://img.shields.io/badge/-Node.js-339933?style=flat-square&logo=node.js&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/-PostgreSQL-336791?style=flat-square&logo=postgresql&logoColor=white)
![Supabase](https://img.shields.io/badge/-Supabase-3ECF8E?style=flat-square&logo=supabase&logoColor=white)

### 🔧 Tools & Technologies

![Git](https://img.shields.io/badge/-Git-F05032?style=flat-square&logo=git&logoColor=white)
![Vite](https://img.shields.io/badge/-Vite-646CFF?style=flat-square&logo=vite&logoColor=white)
![Prisma](https://img.shields.io/badge/-Prisma-2D3748?style=flat-square&logo=prisma&logoColor=white)

---

## 🎯 Key Achievements

- 🚀 **Production Deployment:** Successfully deployed booking platform with commercial recognition
- 🧠 **Algorithmic Excellence:** Solved 757+ unique competitive programming problems
- 📈 **Consistent Performance:** Maintained 77%+ acceptance rate across LeetCode difficulty levels
- 🏗️ **Full-Stack Proficiency:** Built complete applications from database design to frontend
- 💰 **Commercial Recognition:** Project work led to paid opportunity based on technical merit
- 🔧 **Developer Tools:** Built and maintained CP template system used across 56+ contests

---

## 🔬 Areas of Expertise

- **🌐 Full-Stack Development:** End-to-end application development with modern technologies
- **⚡ Algorithm Design:** Strong foundation in competitive programming and optimization
- **🏛️ Database Architecture:** Normalized schema design and efficient query optimization
- **🎨 UI/UX Implementation:** Modern, responsive frontend development
- **🧩 Problem Solving:** Analytical approach to complex technical challenges

---

## 📈 GitHub Stats

![Jai's GitHub stats](https://github-readme-stats.vercel.app/api?username=jaipkapoor99&show_icons=true&theme=radical)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=jaipkapoor99&layout=compact&theme=radical)

---

## 🤖 Leveraging AI in Development

I actively harness **AI-powered tools** and **Large Language Models** to accelerate development and enhance code quality:

- 🚀 Rapidly prototype and validate ideas
- 🔍 Automatically suggest optimizations and best practices
- ⚡ Streamline repetitive tasks and improve productivity
- 📚 Stay current with latest technological advancements

---

## 📫 Let's Connect!

- 💼 **LinkedIn:** [jaipkapoor](https://www.linkedin.com/in/jaipkapoor/)
- 🐙 **GitHub:** [jaipkapoor99](https://github.com/jaipkapoor99)
- 🏆 **Codeforces:** [jaipkapoor](https://codeforces.com/profile/jaipkapoor)
- 💻 **LeetCode:** [jaipkapoor99](https://leetcode.com/jaipkapoor99/)

---

**Always open to collaborations and new opportunities in tech, AI, and full-stack development!**

![Profile Views](https://komarev.com/ghpvc/?username=jaipkapoor99&color=brightgreen)

---

## 🧠 The Mind: Personal Knowledge Base

This repository also serves as a structured personal knowledge base for AI-driven self-reflection and personal growth. It contains organized documentation of experiences, insights, and development plans optimized for AI analysis.

### Repository Structure

- **`Profile/`**: Professional profiles and resume materials
- **`Source/`**: Personal logs, memories, and therapeutic insights
- **`Projects/`**: Detailed project documentation and analysis
- **`Plans/`**: Personal development strategies and goals
- **`CP/`**: Competitive programming profiles and achievements

### Automation & Workflow

The repository features automated workflows for:

- Resume building and cloud synchronization
- Competitive programming profile generation
- File validation and reference checking
- Cloud backup to OneDrive and Google Drive

For more details about the personal knowledge base system, see the documentation in the `docs/` directory.
